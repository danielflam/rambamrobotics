 ...
 */
static inline __attribute__((always_inline))
void fastDigitalWriteHigh(uint8_t pin) {
  badPinCheck(pin);
  fastBitWriteSafe(pinMap[pin].port, pinMap[pin].bit, true);
}
//------------------------------------------------------------------------------
/** Set pin value low
 * @param[in] pin Arduino pin number
 ...
 */
static inline __attribute__((always_inline))
void fastDigitalWriteLow(uint8_t pin) {
  badPinCheck(pin);
  fastBitWriteSafe(pinMap[pin].port, pinMap[pin].bit, false);
}
